...
public class ConsoleLoggerShould
{
    ...
    #region Properties And Exception Overloads

    [Fact]
    public void IncludeBoth_GivenLogTraceWithPropertiesAndException()
    {
        var logger = new ConsoleLogger("Test");
        var props = new Dictionary<string, string> { ["traceKey"] = "traceVal" };
        ...
        {
            Console.SetOut(sw);
            logger.LogTrace(props, ex, "Trace both");
            var output = sw.ToString();
            Assert.Contains("traceKey=traceVal", output);
            Assert.Contains("trace-err", output);
        }
        finally
        {
            Console.SetOut(originalOut);
    ...
    [Fact]
    public void IncludeProperties_GivenLogDebugWithProperties()
    {
        var logger = new ConsoleLogger("Test");
        var props = new Dictionary<string, string> { ["dk"] = "dv" };
        using var sw = new StringWriter();
        var originalOut = Console.Out;
        try
        {
            Console.SetOut(sw);
            logger.LogDebug(props, "Debug props");
            var output = sw.ToString();
            Assert.Contains("dk=dv", output);
        }
        finally
        {
            Console.SetOut(originalOut);
    ...
    [Fact]
    public void IncludeBoth_GivenLogDebugWithPropertiesAndException()
    {
        var logger = new ConsoleLogger("Test");
        var props = new Dictionary<string, string> { ["dk"] = "dv" };
        var ex = new Exception("debug-err");
        using var sw = new StringWriter();
        var originalOut = Console.Out;
        try
        {
            Console.SetOut(sw);
            logger.LogDebug(props, ex, "Debug both");
            var output = sw.ToString();
            Assert.Contains("dk=dv", output);
            Assert.Contains("debug-err", output);
        }
        finally
        {
            Console.SetOut(originalOut);
    ...
    [Fact]
    public void IncludeException_GivenLogInformationWithException()
    {
        var logger = new ConsoleLogger("Test");
        var ex = new Exception("info-err");
        using var sw = new StringWriter();
        var originalOut = Console.Out;
        try
        {
            Console.SetOut(sw);
            logger.LogInformation(ex, "Info with ex");
            var output = sw.ToString();
            Assert.Contains("info-err", output);
        }
        finally
        {
            Console.SetOut(originalOut);
    ...
    [Fact]
    public void IncludeBoth_GivenLogInformationWithPropertiesAndException()
    {
        var logger = new ConsoleLogger("Test");
        var props = new Dictionary<string, string> { ["ik"] = "iv" };
        var ex = new Exception("info-err");
        using var sw = new StringWriter();
        var originalOut = Console.Out;
        try
        {
            Console.SetOut(sw);
            logger.LogInformation(props, ex, "Info both");
            var output = sw.ToString();
            Assert.Contains("ik=iv", output);
            Assert.Contains("info-err", output);
        }
        finally
        {
    ...
    [Fact]
    public void IncludeProperties_GivenLogWarningWithProperties()
    {
        var logger = new ConsoleLogger("Test");
        var props = new Dictionary<string, string> { ["wk"] = "wv" };
        using var sw = new StringWriter();
        var originalOut = Console.Out;
        try
        {
            Console.SetOut(sw);
            logger.LogWarning(props, "Warn props");
            var output = sw.ToString();
            Assert.Contains("wk=wv", output);
        }
        finally
        {
            Console.SetOut(originalOut);
    ...
    [Fact]
    public void IncludeBoth_GivenLogWarningWithPropertiesAndException()
    {
        var logger = new ConsoleLogger("Test");
        var props = new Dictionary<string, string> { ["wk"] = "wv" };
        var ex = new Exception("warn-err");
        using var sw = new StringWriter();
        var originalOut = Console.Out;
        try
        {
            Console.SetOut(sw);
            logger.LogWarning(props, ex, "Warn both");
            var output = sw.ToString();
            Assert.Contains("wk=wv", output);
            Assert.Contains("warn-err", output);
        }
        finally
        {
            Console.SetOut(originalOut);
    ...
    [Fact]
    public void IncludeProperties_GivenLogErrorWithProperties()
    {
        var logger = new ConsoleLogger("Test");
        var props = new Dictionary<string, string> { ["ek"] = "ev" };
        using var sw = new StringWriter();
        var originalOut = Console.Out;
        try
        {
            Console.SetOut(sw);
            logger.LogError(props, "Error props");
            var output = sw.ToString();
            Assert.Contains("ek=ev", output);
        }
        finally
        {
            Console.SetOut(originalOut);
    ...
    [Fact]
    public void IncludeException_GivenLogErrorWithException()
    {
        var logger = new ConsoleLogger("Test");
        var ex = new Exception("err-ex");
        using var sw = new StringWriter();
        var originalOut = Console.Out;
        try
        {
    ...
    [Fact]
    public void IncludeBoth_GivenLogCriticalWithPropertiesAndException()
    {
        var logger = new ConsoleLogger("Test");
        var props = new Dictionary<string, string> { ["ck"] = "cv" };
        var ex = new Exception("crit-err");
        using var sw = new StringWriter();
        var originalOut = Console.Out;
        try
        {
            Console.SetOut(sw);
            logger.LogCritical(props, ex, "Critical both");
            var output = sw.ToString();
            Assert.Contains("ck=cv", output);
            Assert.Contains("crit-err", output);
        }
        finally
        {
            Console.SetOut(originalOut);
    ...
    #endregion Properties And Exception Overloads

    #region FormatMessage Edge Cases

    [Fact]
    public void HandleNullArgs_GivenLogWithNullArgs()
    {
        var logger = new ConsoleLogger("Test");
        using var sw = new StringWriter();
        var originalOut = Console.Out;
        ...
        {
            Console.SetOut(sw);
            logger.LogInformation("No args message");
            var output = sw.ToString();
            Assert.Contains("No args message", output);
        }
        finally
        {
            Console.SetOut(originalOut);
    ...
    [Fact]
    public void ConvertMixedTemplate_GivenStructuredAndPositionalPlaceholders()
    {
        var logger = new ConsoleLogger("Test");
        using var sw = new StringWriter();
        var originalOut = Console.Out;
        ...
        {
            Console.SetOut(sw);
            logger.LogInformation("{Name} has {Count} items", "Alice", 5);
            var output = sw.ToString();
            Assert.Contains("Alice", output);
    ...
    #endregion FormatMessage Edge Cases
}
