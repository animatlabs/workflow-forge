...
public class MiddlewareShould
{
    ...
    [Fact]
    public async Task ExecuteWithoutTimeout_GivenOperationTimeoutIsZero()
    {
        // Arrange
        var foundry = CreateTestFoundry();
        var logger = Mock.Of<IWorkflowForgeLogger>();
        var timeoutMiddleware = new OperationTimeoutMiddleware(TimeSpan.Zero, logger);
        var operation = new DelegateWorkflowOperation<object, string>("NoTimeoutOp", async (input, f, ct) =>
        {
            await Task.Yield();
            return "ok";
        });
        ...
        // Act
        var result = await timeoutMiddleware.ExecuteAsync(
            operation,
            foundry,
            new object(),
        ...
        Assert.False(foundry.Properties.ContainsKey(FoundryPropertyKeys.OperationTimedOut));
    }

    [Fact]
    public async Task UseCustomTimeout_GivenOperationTimeoutPropertyIsSet()
    {
        // Arrange
        var foundry = CreateTestFoundry();
        var logger = Mock.Of<IWorkflowForgeLogger>();
        var timeoutMiddleware = new OperationTimeoutMiddleware(TimeSpan.FromSeconds(1), logger);
        var operation = new DelegateWorkflowOperation<object, string>("CustomTimeoutOp", async (input, f, ct) =>
        {
            await Task.Delay(100, ct);
            return "late";
        });
        foundry.Properties[FoundryPropertyKeys.CurrentOperationIndex] = 0;
        foundry.Properties[string.Format(FoundryPropertyKeys.OperationTimeoutFormat, 0, operation.Name)] = TimeSpan.FromMilliseconds(25);
        ...
        Assert.True(foundry.Properties.TryGetValue(FoundryPropertyKeys.OperationTimeoutDuration, out var timeoutDuration));
        Assert.Equal(TimeSpan.FromMilliseconds(25), timeoutDuration);
    }

    [Fact]
    public async Task ThrowOperationCanceledException_GivenOperationCancellationRequestedBeforeTimeout()
    {
        // Arrange
        var foundry = CreateTestFoundry();
        var logger = Mock.Of<IWorkflowForgeLogger>();
        var timeoutMiddleware = new OperationTimeoutMiddleware(TimeSpan.FromMilliseconds(200), logger);
        var operation = new DelegateWorkflowOperation<object, string>("CanceledOp", async (input, f, ct) =>
        {
    ...
    [Fact]
    public void ThrowArgumentException_GivenNegativeOperationTimeout()
    {
        // Arrange
        var logger = Mock.Of<IWorkflowForgeLogger>();

        // Act and Assert
        Assert.Throws<ArgumentException>(() => new OperationTimeoutMiddleware(TimeSpan.FromMilliseconds(-1), logger));
    }

    [Fact]
    public void ThrowArgumentNullException_GivenNullLoggerForOperationTimeoutMiddleware()
    {
        // Act and Assert
        Assert.Throws<ArgumentNullException>(() => new OperationTimeoutMiddleware(TimeSpan.FromSeconds(1), null!));
    }
