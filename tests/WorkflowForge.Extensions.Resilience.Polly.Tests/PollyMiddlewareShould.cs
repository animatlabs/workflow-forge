...
public class PollyMiddlewareShould : IDisposable
{
    ...
    [Fact]
    public void SetCustomName_GivenWithCircuitBreakerPolicyCustomName()
    {
        var logger = TestNullLogger.Instance;

        var middleware = PollyMiddleware.WithCircuitBreakerPolicy(logger, name: "CustomCB");

        Assert.Equal("CustomCB", middleware.Name);
    }

    [Fact]
    public void SetCustomName_GivenWithTimeoutPolicyCustomName()
    {
        var logger = TestNullLogger.Instance;

        var middleware = PollyMiddleware.WithTimeoutPolicy(logger, TimeSpan.FromSeconds(5), name: "CustomTimeout");

        Assert.Equal("CustomTimeout", middleware.Name);
    }

    [Fact]
    public void SetCustomName_GivenWithComprehensivePolicyCustomName()
    {
        var logger = TestNullLogger.Instance;

        var middleware = PollyMiddleware.WithComprehensivePolicy(logger, name: "CustomComp");

        Assert.Equal("CustomComp", middleware.Name);
    }

    [Fact]
    public async Task ReturnResult_GivenExecuteAsyncWithComprehensivePolicyAllEnabled()
    {
        var logger = TestNullLogger.Instance;
        var middleware = PollyMiddleware.WithComprehensivePolicy(
            logger,
        ...
        Task<object?> Next(CancellationToken _) => Task.FromResult<object?>("comprehensive-ok");
    ...
    [Fact]
    public async Task RetryWithComprehensivePolicy_GivenTransientFailure()
    {
        var logger = TestNullLogger.Instance;
        var middleware = PollyMiddleware.WithComprehensivePolicy(
            logger,
        ...
        Task<object?> Next(CancellationToken _)
        {
            ...
            if (attempts < 2)
                throw new InvalidOperationException("transient");
            return Task.FromResult<object?>("recovered");
        }
    ...
    [Fact]
    public async Task RetryAndLogWithException_GivenRetryPolicyWithBaseAndMaxDelay()
    {
        var logger = TestNullLogger.Instance;
        var middleware = PollyMiddleware.WithRetryPolicy(
            logger,
    ...
    [Fact]
    public async Task ThrowGenericException_GivenExecuteAsyncWithComprehensivePolicyNonRecoverableFailure()
    {
        var logger = TestNullLogger.Instance;
        var middleware = PollyMiddleware.WithComprehensivePolicy(
            logger,
    ...
    [Fact]
    public void UseDefaultName_GivenWithRetryPolicyDefaultParams()
    {
        var logger = TestNullLogger.Instance;

        var middleware = PollyMiddleware.WithRetryPolicy(logger);

        Assert.Equal("PollyRetry(attempts:3)", middleware.Name);
    }

    [Fact]
    public void UseDefaultName_GivenWithCircuitBreakerPolicyDefaultParams()
    {
        var logger = TestNullLogger.Instance;
