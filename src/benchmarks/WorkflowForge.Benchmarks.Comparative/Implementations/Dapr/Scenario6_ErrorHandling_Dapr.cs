...
public class Scenario6_ErrorHandling_Dapr : IWorkflowScenario
{
    private readonly ScenarioParameters _parameters;

    public string Name => "Error Handling Workflow";
    public string Description => "Execute workflow with error handling and compensation using Dapr Workflow";

    public Scenario6_ErrorHandling_Dapr(ScenarioParameters parameters) => _parameters = parameters;
    ...
    public async Task<ScenarioResult> ExecuteAsync()
    {
        ...
        for (var i = 0; i < _parameters.OperationCount; i++)
        {
            ...
            {
                await SimulateCompensationAsync(i);
                operationsExecuted++;
            }
        }
    ...
    {
        if (step % 5 == 4) throw new InvalidOperationException("Simulated error");
        return Task.CompletedTask;
    }

    private static Task SimulateCompensationAsync(int step) { _ = step; return Task.CompletedTask; }
}
#endif
